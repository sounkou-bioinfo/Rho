...
collected; `rho_await()` is the explicit wait.
...
```{r codex-agent, eval = isTRUE(getOption("rho.codex_readme_rebuild", FALSE))}
library(rho.coding)
...
Provider implementations with no independent dependency or ABI constraint live
...
without a package per API. The [Pi parity ledger](https://rgenomicsetl.github.io/Rho/docs/pi-parity.html) distinguishes
...
executable normalized-stream fixtures; the ledger records the executable and
...
Rho targets R 4.4 or newer. Install the development monorepo from a checkout:
...
The [Pi parity ledger](https://rgenomicsetl.github.io/Rho/docs/pi-parity.html) records behavioral contracts and the
fixtures that verify them. Public release and addition to the
[RGenomicsETL R-universe](https://rgenomicsetl.r-universe.dev) follow
green package checks, live provider checks, documentation, and secret scanning.
The exact sequence is recorded in the [publishing guide](https://rgenomicsetl.github.io/Rho/docs/releasing.html).
