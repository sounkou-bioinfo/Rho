...
## Usage estimates combine provider accounting with trailing messages
...
## Complete request accounting includes stable prompt material

```{r complete-context-accounting}
...
## Provider counts belong to one request-context revision

```{r context-revision-accounting}
...
## Assistant usage records the request context it measured

```{r assistant-context-revision}
...
## Manual compaction preserves the transcript and projects a checkpoint

```{r generated-compaction}
...
## Threshold compaction uses the configured compactor

```{r threshold-compaction}
provider <- rho_compaction_fixture_provider(rep(
  list(rho_compaction_fixture_message()),
  2L
))
...
## Policy methods may provide the result and observe its durable entry
...
## Cancellation is a typed policy outcome

```{r policy-cancels-compaction}
...
## Automatic compaction does not conceal an operational failure

```{r threshold-compaction-failure}
...
## Provider input limits record the failed response and retry once

```{r provider-input-limit-recovery}
...
```{r one-provider-input-limit-retry}
provider <- rho_compaction_fixture_provider(list(
  rho_compaction_fixture_message("history one"),
  rho_compaction_fixture_message("history two"),
  overflow,
...
## A retained context never begins with a tool result
...
## A compaction cut belongs to the selected branch
