...
## The agent closes over a structural session journal
...
## Recorded order and conversation lineage remain distinct

```{r session-branch-lineage}
journal <- rho_memory_session_journal()
agent <- rho_agent(
  rho_faux_provider(),
  rho_model("faux", "faux"),
...
## An agent synchronizes an existing journal without replaying partial state

```{r session-journal-synchronization}
journal <- rho_memory_session_journal()
writer <- rho_agent(
  rho_faux_provider(),
  rho_model("faux", "faux"),
...
```{r session-journal-compare-and-append}
journal <- rho_memory_session_journal()
...
```{r session-journal-failure-is-a-value}
...
```{r provider-operation-is-not-local-tool-call}
executions <- 0L
