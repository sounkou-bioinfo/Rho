...
FixtureEmbeddedProvider <- S7::new_class("FixtureEmbeddedProvider")

S7::method(
  rho_provider_transports,
  list(FixtureEmbeddedProvider, Model)
...
S7::method(
  ...
  list(EmbeddedTransport, FixtureEmbeddedProvider, Model, Context)
  ...
  rho_list_stream(list(rho_assistant_done_event(answer)))
}

embedded_provider <- FixtureEmbeddedProvider()
...
expect_true(S7::S7_inherits(automatic_transport@transport, SseTransport))
expect_true(S7::S7_inherits(required_websocket, ProviderTransportUnsupported))
