...
7. Blocking is allowed only in `rho_await()`, at the CLI top level, and in test helpers.
...
37. Provider request bodies are composed from typed policy or section values. Do not assemble optional wire fields with long `if`/`else` chains or use provider strings as internal state. S7 methods translate typed values to strings while encoding the request, and the final body builder reduces the resulting named sections.
...
40. Operational failures are typed values. Installable package code does not use a package-local `rho_abort()` wrapper or bare `stop()` for transport, provider, tool, compute, graphics, SQL, cancellation, or timeout outcomes. `rho_await()` may re-signal an explicitly rejected task during an explicit wait; build scripts and tests may stop their process.
...
44. Name every operation or constraint directly: request encoding, explicit wait, worker selection, session checkpoint, ABI constraint, or dispatch. Do not substitute a generic architecture noun.
45. Provider protocol selection is derived from a declared endpoint or an explicit typed descriptor. Model IDs, family names, and vendor names are not protocol evidence. A missing declaration stays unavailable; an unknown declaration fails catalog compilation.
46. `packages/rho.ai/data-raw/model-registry.R` is the single authored provider-catalog manifest. Store provider and protocol constructors as R language objects, derive updater source IDs from that manifest, and validate projected endpoint declarations there. Do not add a parallel string-to-class table, a model-name router, or generated R source containing one constructor per model.
